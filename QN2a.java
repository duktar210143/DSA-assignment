...
// with edge[i]=[xi,yi], where node xi is connected to xj. You are also provided an array
//  of values associated with each species, such that value[i] reflects the ith nodes value.
...
//   ancestor of certain species i. Return a list of nearest ancestors, where result[i] is the node 
//   i's nearest ancestor such that values[i] and value[result[i]] are both relative primes otherwise -1.
class Solution {
